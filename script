...
local function ResizeMain()
	...
	local Width = math.clamp(Viewport.X * 0.75, 700, 950)
	local Height = math.clamp(Viewport.Y * 0.75, 450, 580)
	Main.Size = UDim2.fromOffset(Width, Height)
	Main.Position = UDim2.new(0.5,0,0.5,0)
end
ResizeMain()
workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(ResizeMain)
...
Title.Text = "VanDayModZ v3.0 | ESP Edition"
...
local currentWidth = Main.Size.X.Offset
local currentHeight = Main.Size.Y.Offset

CloseBtn.MouseButton1Click:Connect(function()
    TweenService:Create(Main, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {Size = UDim2.new(0, currentWidth, 0, 0)}):Play()
    task.wait(0.35)
    ScreenGui:Destroy()
end)
...
MinBtn.MouseButton1Click:Connect(function()
    ...
    if minimized then
        TweenService:Create(Main, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {Size = UDim2.new(0, Main.Size.X.Offset, 0, 42)}):Play()
    else
        TweenService:Create(Main, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {Size = UDim2.new(0, Main.Size.X.Offset, 0, currentHeight)}):Play()
    end
end)
...
SidebarLine.Position = UDim2.new(0.22,0,0,0)
...
Content.Parent = Body
Content.Position = UDim2.new(0.22,1,0,0)
Content.Size = UDim2.new(0.78,-1,1,0)
...
local function openPage(name)
    ...
    if Pages[name] then
        ...
        if btn then
            ...
            TweenService:Create(btn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(25,25,45)}):Play()
            for _, c in ipairs(btn:GetChildren()) do
...
local function makeRow(page, labelText, descText)
    ...
    LabelMain.Position = UDim2.new(0, 18, 0, 12)
    LabelMain.Size = UDim2.new(0.6, 0, 0, 22)
...
local function addButton(page, label, desc, btnText, callback)
    ...
    Btn.MouseEnter:Connect(function()
        TweenService:Create(Btn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(120, 120, 255)}):Play()
    end)
    Btn.MouseLeave:Connect(function()
        TweenService:Create(Btn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(95, 95, 255)}):Play()
    end)
    Btn.MouseButton1Click:Connect(function()
       TweenService:Create(Btn, TweenInfo.new(0.08), {BackgroundColor3 = Color3.fromRGB(70, 70, 200)}):Play()
        task.wait(0.1)
        TweenService:Create(Btn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(95, 95, 255)}):Play()
...
local function addToggle(page, label, desc, default, callback)
    ...
    Circle.Position = state and UDim2.new(1, -25, 0.5, -11) or UDim2.new(0, 3, 0.5, -11)
    ...
    ToggleBG.MouseButton1Click:Connect(function()
        ...
        TweenService:Create(ToggleBG, TweenInfo.new(0.2), {
            BackgroundColor3 = state and Color3.fromRGB(95, 95, 255) or Color3.fromRGB(45, 45, 65)
        }):Play()
...
local function addSlider(page, label, desc, min, max, default, callback)
    ...
    local LabelMain = Instance.new("TextLabel")
    LabelMain.Parent = Row
    LabelMain.Position = UDim2.new(0, 18, 0, 10)
    ...
    ValueLabel.Position = UDim2.new(1, -18, 0, 10)
    ...
    local Desc = Instance.new("TextLabel")
    Desc.Parent = Row
    Desc.Position = UDim2.new(0, 18, 0, 30)
    Desc.Size = UDim2.new(1, -36, 0, 16)
    Desc.BackgroundTransparency = 1
    Desc.Text = desc
    Desc.TextColor3 = Color3.fromRGB(90, 90, 120)
    Desc.Font = Enum.Font.Gotham
    ...
    Bar.Parent = Row
    Bar.Position = UDim2.new(0, 18, 0, 55)
...
PAccent.Size = UDim2.new(0, 3, 0.7, 0)
PAccent.Position = UDim2.new(0, 0, 0.15, 0)
...
AvatarBG.Size = UDim2.new(0, 66, 0, 66)
AvatarBG.Position = UDim2.new(0, 20, 0.5, -33)
...
DisplayLabel.Position = UDim2.new(0, 100, 0, 38)
DisplayLabel.Size = UDim2.new(1, -110, 0, 18)
...
IDLabel.Position = UDim2.new(0, 100, 0, 56)
IDLabel.Size = UDim2.new(1, -110, 0, 18)
IDLabel.BackgroundTransparency = 1
IDLabel.Text = "User ID: " .. LocalPlayer.UserId
IDLabel.TextColor3 = Color3.fromRGB(100, 100, 140)
...
AgeLabel.Position = UDim2.new(0, 100, 0, 74)
AgeLabel.Size = UDim2.new(1, -110, 0, 18)
AgeLabel.BackgroundTransparency = 1
AgeLabel.Text = "Account Age: N/A"
AgeLabel.TextColor3 = Color3.fromRGB(100, 100, 140)
AgeLabel.Font = Enum.Font.Gotham
AgeLabel.TextSize = 12
AgeLabel.TextXAlignment = Enum.TextXAlignment.Left

addSection(MainPage, "Profile")
addSection(MainPage, "Main")
addButton(MainPage, "Button", "Click button", "Click", function()
    print("Button clicked!")
end)
addToggle(MainPage, "Toggle", "Toggle feature", false, function(val)
    print("Toggle:", val)
end)
addSlider(MainPage, "Slider", "Adjust value", 0, 100, 30, function(val)
    print("Slider:", val)
end)
...
addButton(MiscPage, "Rejoin", "Rejoin current server", "Rejoin", function()
    local TeleportService = game:GetService("TeleportService")
    TeleportService:Teleport(game.PlaceId, LocalPlayer)
end)
addToggle(MiscPage, "Anti-AFK", "Prevent auto kick", true, function(val)
    print("Anti-AFK:", val)
end)
addSlider(MiscPage, "WalkSpeed", "Set walk speed", 16, 100, 16, function(val)
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
        LocalPlayer.Character.Humanoid.WalkSpeed = val
    end
end)
addSlider(MiscPage, "JumpPower", "Set jump power", 50, 300, 50, function(val)
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
...
-- =============================================================================
-- NEW: PAGE ESP NÂNG CẤP HOÀN CHỈNH
-- =============================================================================
...
local function CreateESPStorage(player)
	...
	store.CornerContainer.Parent = ScreenGui
...
local function RemoveESPStorage(player)
	if ESP_Cache[player] then
		...
		if store.CornerContainer then store.CornerContainer:Destroy() end
		if store.HealthBG then store.HealthBG:Destroy() end
		if store.Tracer then store.Tracer:Destroy() end
		if store.Distance then store.Distance:Destroy() end
		if store.Highlight then store.Highlight:Destroy() end
		if store.Skeleton then
		...
		if character and rootPart and humanoid and humanoid.Health > 0 and allowedByTeamCheck then
			...
			if onScreen then
				...
				if ESP_CornerBoxes then
					...
					local thick = 1.5
					local len = math.clamp(boxWidth * 0.2, 5, 15)
				...
				if ESP_HealthBar then
					...
					store.HealthMain.Position = UDim2.new(0, 0, 1 - healthPct, 0)
				...
				if ESP_Skeleton then
					...
					if head and torso and leftHand and rightHand and leftFoot and rightFoot then
						...
						local skColor = Color3.fromRGB(255, 255, 255)
						...
						DrawLineBetweenPoints(store.Skeleton[4], pTorso, Vector2.new((pLFoot.X + pRFoot.X)/2, (pLFoot.Y + pRFoot.Y)/2), skColor)

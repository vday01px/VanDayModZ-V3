...
-- Hàm tối ưu cấu hình nhanh cho các đối tượng Instance UI (Đã sửa độc lập hoàn hảo)
local function configureInstance(instance, properties)
		...
		if character and rootPart and humanoid and humanoid.Health > 0 and allowedByTeamCheck then
			...
			if onScreen then
				...
				if ESP_CornerBoxes then
					...
					-- Góc trên bên trái
					configureInstance(store.Corners[1], {Position = UDim2.fromOffset(topX, topY), Size = UDim2.fromOffset(len, thick), BackgroundColor3 = renderColor, Visible = true})
					configureInstance(store.Corners[2], {Position = UDim2.fromOffset(topX, topY), Size = UDim2.fromOffset(thick, len), BackgroundColor3 = renderColor, Visible = true})
					-- Góc trên bên phải
					configureInstance(store.Corners[3], {Position = UDim2.fromOffset(topX + boxWidth - len, topY), Size = UDim2.fromOffset(len, thick), BackgroundColor3 = renderColor, Visible = true})
					configureInstance(store.Corners[4], {Position = UDim2.fromOffset(topX + boxWidth - thick, topY), Size = UDim2.fromOffset(thick, len), BackgroundColor3 = renderColor, Visible = true})
					-- Góc dưới bên trái
					configureInstance(store.Corners[5], {Position = UDim2.fromOffset(topX, topY + boxHeight - thick), Size = UDim2.fromOffset(len, thick), BackgroundColor3 = renderColor, Visible = true})
					configureInstance(store.Corners[6], {Position = UDim2.fromOffset(topX, topY + boxHeight - len), Size = UDim2.fromOffset(thick, len), BackgroundColor3 = renderColor, Visible = true})
					-- Góc dưới bên phải
					configureInstance(store.Corners[7], {Position = UDim2.fromOffset(topX + boxWidth - len, topY + boxHeight - thick), Size = UDim2.fromOffset(len, thick), BackgroundColor3 = renderColor, Visible = true})
					configureInstance(store.Corners[8], {Position = UDim2.fromOffset(topX + boxWidth - thick, topY + boxHeight - len), Size = UDim2.fromOffset(thick, len), BackgroundColor3 = renderColor, Visible = true})
				else
					store.CornerContainer.Visible = false
				end
